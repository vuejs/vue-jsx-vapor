...
pub fn gen_insert_node<'a>(oper: InsertNodeIRNode, context: &CodegenContext<'a>) -> Statement<'a> {
  ...
  if elements.len() > 1 {
    ...
    arguments.push(
      ast
        .expression_identifier(SPAN, ast.str(&format!("_n{}", elements[0])))
        .into(),
    )
  }
  ...
  arguments.push(
    ast
      .expression_identifier(SPAN, ast.str(&format!("_n{parent}")))
      .into(),
  );
  ...
  if let Some(anchor) = anchor {
    arguments.push(
      ast
        .expression_identifier(SPAN, ast.str(&format!("_n{anchor}")))
        .into(),
    );
  }
