...
export function walkIdentifiers(
  ...
  walkAST<Node & { scopeIds?: Set<string> }>(root, {
    enter(node, parent) {
      ...
      if (node.type === 'Identifier' || node.type === 'JSXIdentifier') {
        ...
        const isRefed = isReferencedIdentifier(node, parent as any, parentStack)
        if (includeAll || (isRefed && !isLocal)) {
          onIdentifier(node, parent as any, parentStack, isRefed, isLocal)
        }
      } else if (
