...
fn root_slot_outlet_in_slot_content_should_not_be_marked_as_slot_root() {
  ...
  (() => {
  	const _n1 = _createComponent(Comp, null, _extend(() => {
  		const _n0 = _createSlot("default", null, null, 6);
  		return _n0;
  	}, { _: 1 }), true);
  	return _n1;
  })();
...
#[test]
fn with_v_show() {
  let code = transform("<div v-show={show} v-once />", None).code;
...
#[test]
fn with_v_model() {
  let code = transform("<input v-model={text} v-once />", None).code;
...
fn with_custom_directive() {
  let code = transform("<div v-dir={val} v-once />", None).code;
...
fn directives_outside_v_once_are_not_wrapped() {
  let code = transform("<><div v-show={show} v-dir={val} /><div v-once /></>", None).code;
